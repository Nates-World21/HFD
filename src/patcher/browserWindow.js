...
class PatchedBrowserWindow extends BrowserWindow {
  constructor (opts) {
    ...
    win.on('maximize', () => void win.webContents.send('HFD_WINDOW_MAXIMIZE'));
    win.on('unmaximize', () => void win.webContents.send('HFD_WINDOW_UNMAXIMIZE'));

    win.webContents._hfdPreload = originalPreload;
    return win;
  }

  static loadUrl (ogLoadUrl, url, opts) {
    const match = url.match(/^https:\/\/((?:canary|ptb)\.)?discord(app)?\.com\/_hfd\//);
    if (match) {
      this.webContents._HfdOgUrl = url;
